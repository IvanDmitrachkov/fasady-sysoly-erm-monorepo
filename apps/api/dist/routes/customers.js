...
export const customersRoutes = async (app) => {
    ...
    app.post("/customers", { preHandler: [requireRoles(Role.ADMIN)] }, async (request, reply) => {
        ...
        await writeAudit(app.prisma, authUserId(request), "customer.create", `Создан заказчик «${customer.name}»`, "Customer", customer.id);
        return reply.code(201).send({ customer });
    });
    app.patch("/customers/:id", { preHandler: [requireRoles(Role.ADMIN)] }, async (request, reply) => {
        ...
        await writeAudit(app.prisma, authUserId(request), "customer.update", `Заказчик «${existing.name}» → «${customer.name}»`, "Customer", customer.id);
        return { customer };
    });
};
