...
export async function ensureTimeEntrySqlite(prisma) {
    ...
    if (!url.startsWith("file:"))
        return;
    const rows = await prisma.$queryRawUnsafe("SELECT name FROM sqlite_master WHERE type='table' AND name='TimeEntry'");
    if (rows.length > 0)
        return;
    await prisma.$executeRawUnsafe(CREATE_TIME_ENTRY);
}
//# sourceMappingURL=ensure-time-entry-sqlite.js.map